...
module.exports = class HTMLCreator {
  ...
  create(key, value, dev) {
    return this.renderFile(value, dev).then(template => {
      return {
        key: key,
  ...
  renderFile(pageLocation, dev) {
    return access.readPageData(pageLocation).then(data => {
      ...
      return this.resolveElements(element, 0, null, null, dev).then(html => {
        if(dev) {
  ...
  resolveElements(element, depth = 0, nextKey, parentKey, dev) {
    ...
    if (depth > 0) {
      ...
      //if(dev) $(rootChildren[0]).attr('dev', parentKey);
    }
    this.insertContent(element.content, $, {
      key: dev ? parentKey : null
    });
    ...
    $('*').each((_, elem) => {
      for (let [name, value] of keva(elem.attribs)) {
        ...
        if (match) {
          ...
          if(options.key) {
            $(elem).attr(`dev-${name}`, `${options.key} ${value}`);
          }
        }
      }
    });
