'use strict';

const decb = require('decb');
const fs = decb(require('fs'), {
  use: ['readFile']
});
const cheerio = require('cheerio');
const root = require('app-root-path').path + '/';
const keva = require('keva');
const merge = require('merge');

const modules = {
  'html-content': [],
  'json-content': []
};
const defaultOpt = {
  key: 'key.json',
  folders: {
    organism: 'templates/organisms',
    molecule: 'templates/molecules',
    atom: 'templates/atoms'
  },
  content: 'content',
  pages: 'templates/pages',
  attributes: {
    name: 'name',
    type: 'type',
    content: 'content'
  }
};

module.exports = class Atomic {
  constructor(opt = {}) {
    this.opt = merge.recursive(defaultOpt, opt);
    this._modules = modules;
    this._createRequestHandler();
    this._createTemplates().then(templates => {
      this.templates = templates;
      this.onLoad();
    });
  }

  isDone(onLoad) {
    this.onLoad = onLoad;
  }

  _createRequestHandler() {
    let cms = this;
    this.requestHandler = function(req, res, next) {
      let target = cms.templates[req.url];
      if (target) {
        res.send(target);
      } else {
        next();
      }
    };
  }

  addModule() {
    let target;
    let module;
    if (typeof arguments[0] === 'object') {
      target = arguments[0].target;
      module = arguments[0].module;
    } else {
      target = arguments[0];
      module = arguments[1];
    }

    if (!this._modules[target]) throw new Error('Event does not exist.');
    if (typeof module !== 'function') throw new Error('Not a function.');
    this._modules[target].push(module);
  }

  _createTemplates() {
    let keyPath = `${root}${this.opt.key}`;
    return fs.readFile(keyPath, 'utf8').then(JSON.parse).then(keyJson => {
      let templates = {};
      let promises = [];
      for (let [key, value] of keva(keyJson)) {
        if (typeof value === 'string') {
          value = {
            template: value + '.html',
            content: value + '.json'
          };
        }
        promises.push(this._renderFile(value.template, value.content)
        .then(template => {
          return {key: key, template: template};
        }));
      }

      return Promise.all(promises).then(rendered => {
        rendered.forEach(render => {
          templates[render.key] = render.template;
        });
        return templates;
      });
    });
  }

  _renderFile(templatePath, contentPath) {
    templatePath = `${root}${this.opt.pages}/${templatePath}`;
    contentPath = `${root}${this.opt.content}/${contentPath}`;
    return Promise.all([
      fs.readFile(templatePath, 'utf8'),
      fs.readFile(contentPath, 'utf8')
    ]).then(values => {
      let [template, content] = values;
      return this._resolveElements(template, JSON.parse(content));
    });
  }

  _resolveElements(template, content, depth = 0) {
    let $ = cheerio.load(template, {
      xmlMode: true
    });
    template = this._insertContent(content, $);

    if(depth > Object.keys(this.opt.folders).length - 1) {
      return template;
    }

    let promises = [];
    let level = Object.keys(this.opt.folders)[depth];

    $(level).each((_, elem) => {
      let name = $(elem).attr(`${this.opt.attributes.name}`);
      let type = $(elem).attr(`${this.opt.attributes.type}`);
      let currentContent = content[name || type];

      for (let [name, value] of keva(elem.attribs)) {
        let match = name.match(new RegExp(`^cms-(\w+)`));
        if (match) currentContent[match[1]] = value;
      }

      promises.push(
        fs.readFile(`${root}/${this.opt.folders[level]}/${type}.html`, 'utf8')
        .then(template => {
          return this._resolveElements(template, currentContent, depth + 1);
        }).then(html => ({node: elem, html: html})));
    });

    return Promise.all(promises).then(data => {
      data.forEach(elem => {
        $(elem.node).replaceWith(elem.html);
      });
      return $.html();
    });
  }

  _insertContent(data, $) {
    $('*').each((_, elem) => {
      for (let [name, value] of keva(elem.attribs)) {
        let match = name.match(/^cms-(\w+)/);
        if (match) {
          $(elem).removeAttr(name);
          if (match[1] === this.opt.attributes.content) {
            let html = data[value];
            html = this._applyModules(html, 'html-content');
            $(elem).html(html);
          } else {
            $(elem).attr(match[1], data[value]);
          }
        }
      }
    });

    return $.html();
  }

  _applyModules(target, name) {
    if (this._modules[name].length > 0) {
      return this._modules[name].reduce((a, b) => {
        return b(a);
      }, target);
    }
    return target;
  }
};
