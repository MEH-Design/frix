...
function renderFile(templatePath, contentPath) {
  templatePath = `${opt.root}${opt.pages}/${templatePath}`;
...
function createTemplates() {
  prefixes = require(`../${opt.root}${opt.prefix}`);
  ...
  .then(JSON.parse).then(keyJson => {
    ...
    for (let [key, value] of keva(keyJson)) {
      ...
      .then(template => {
        return {key: key, template: template, file: `${opt.root}bin/` + value.template};
      }));
    }
...
function resolveElements(template, content, depth = 0) {
  ...
  $(level).each((_, elem) => {
    ...
    promises.push(
      fs.readFile(`${opt.root}${opt.folders[level]}/${type}.html`, 'utf8')
      .then(template => {
