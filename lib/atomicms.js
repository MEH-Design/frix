...
module.exports = class Atomic {
  ...
  addModule() {
    ...
    if(!this._modules[target]) throw 'Event does not exist.';
    if(typeof module !== 'function') throw 'Not a function.';
  ...
  _resolveElements(template, content, depth = 0) {
    ...
    $(level).each(function() {
      let name = this.attr(`${this.opt.attributes.name}`);
      let type = this.attr(`${this.opt.attributes.type}`);
